...
printf "\n"
info "Where are the source photos you want to organize?"
info "This path is mounted read-only at /mnt/source."
printf "${BOLD}  Source photo path${NC} [default: $HOME/Pictures]: "
read -r SOURCE_PATH
SOURCE_PATH="${SOURCE_PATH:-$HOME/Pictures}"
printf "${BOLD}  Organized destination path${NC} [default: $HOME/Pictures-Organized]: "
read -r DESTINATION_PATH
DESTINATION_PATH="${DESTINATION_PATH:-$HOME/Pictures-Organized}"
...
if [ ! -d "$SOURCE_PATH" ]; then
  warn "Source directory does not exist yet: $SOURCE_PATH"
  printf "  Create it? [Y/n]: "
  read -r CREATE_DIR
  CREATE_DIR="${CREATE_DIR:-Y}"
  if [[ "$CREATE_DIR" =~ ^[Yy] ]]; then
    mkdir -p "$SOURCE_PATH"
    ok "Created $SOURCE_PATH"
  else
...
# ── Authentication ────────────────────────────────────────
printf "${BOLD}  Web username${NC} [default: admin]: "
read -r AUTH_USERNAME
AUTH_USERNAME="${AUTH_USERNAME:-admin}"
while true; do
  printf "${BOLD}  Web password${NC} [minimum 16 characters]: "
  read -rs AUTH_PASSWORD
  printf "\n"
  [ "${#AUTH_PASSWORD}" -ge 16 ] && break
  warn "Password must contain at least 16 characters."
done
...
# ── Write .env consumed by hardened docker-compose.yml ─────
dotenv_escape() {
  local value="$1"
  value="${value//\\/\\\\}"
  value="${value//\"/\\\"}"
...
SNAPSORT_SOURCE_PATH=$(dotenv_escape "$SOURCE_PATH")
SNAPSORT_DESTINATION_PATH=$(dotenv_escape "$DESTINATION_PATH")
SNAPSORT_AUTH_USERNAME=$(dotenv_escape "$AUTH_USERNAME")
SNAPSORT_AUTH_PASSWORD=$(dotenv_escape "$AUTH_PASSWORD")
SNAPSORT_BIND_ADDRESS=127.0.0.1
SNAPSORT_PORT=${PORT}
SNAPSORT_VERSION=$(cat VERSION)
ENV

if [ -f "$INSTALL_DIR/docker-compose.override.yml" ]; then
  mv "$INSTALL_DIR/docker-compose.override.yml" "$INSTALL_DIR/docker-compose.override.yml.legacy"
  warn "Moved legacy writable-mount override to docker-compose.override.yml.legacy"
fi
ok "Secure configuration saved to .env"
...
info "Source:      ${BOLD}${SOURCE_PATH}${NC} (read-only)"
info "Destination: ${BOLD}${DESTINATION_PATH}${NC}"
